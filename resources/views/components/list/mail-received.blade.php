...
<li class="flex my-2 rounded-lg hover:bg-slate-100 hover:cursor-pointer">
    <button class="mail-received-item grow flex flex-col p-1"value="{{ $item['mail']->id }}">
        <p class="text-xs text-stone-600">{{ Carbon\Carbon::parse($item['mail']->created_at)->format('H:i:s d/m/Y') }}
        </p>
        <p class="text-sm text-stone-700 line-clamp-1 text-ellipsis break-words">From:
            thisisbaomail@gmail.com
        </p>
